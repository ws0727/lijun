...
*                                            ]
    ...
    /**
     * 开关原图保护
     * node 为1表示开启原图保护，0表示关闭
     */
    public function putBucketAccessStyleMode($bucket, $mode)
    {
        $path = '/accessMode/' . $bucket . '/node/' . $mode;
    ...
    {
        $path = "/referAntiLeech?bucket=$bucket&node=$mode&norefer=$norefer&pattern=$pattern&source_enabled=$enabled";
        $info = $this->ucPost($path, null);
        return $info;
    }
