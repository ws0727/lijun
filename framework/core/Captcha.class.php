...
class Captcha{
    ...
    //构造方法， 三个参数
    function __construct($width=150, $height=32, $num=3) {
        $this->width = $width;
        $this->height = $height;
        $this->num = $num;
    }
    ...
    public function checkCode($input_code){
        ...
        if($code){
            if(time()-$code['time']>60){
                ...
                if(strtolower($code['code'])==strtolower($input_code)){
                    return true;
                }else{
                    return false;
                }
    ...
    private function createcode() {
        ...
        Session::add("code",["code"=>$code,"time"=>time()]);
        return $code;
    }
}
